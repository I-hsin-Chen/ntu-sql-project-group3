...
def greedy_selective_pairwise_join(query):
    ...
    while True:
        ...
        for node_pair in node_pairs:
            ...
            import random
            # for testing purposes, generate random cardinalities
        ...
        for condition in conditions:
            if condition in node_pairs[best_combination]:
                conditions.remove(condition)
            elif table1[1] in condition:
                condition = condition.replace(f"{table1[1]}.", f"{combined_alias}.")
            elif table2[1] in condition:
                condition = condition.replace(f"{table2[1]}.", f"{combined_alias}.")

        if len(tables) == 2:
            final_query = f'SELECT COUNT(*) FROM {tables[0][0]} {tables[0][1]}, {tables[1][0]} {tables[1][1]} WHERE {condition};'
            print(final_query)
            break
