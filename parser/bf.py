    ...
    def brutal_force(tables, conditions):
        ...
        for table1, table2 in list(itertools.combinations(tables, 2)):
            ...
            if node_pairs[(table1, table2)] == []:
                raise Exception(f'Parsing error : No conditions found for joining {
                                table1} and {table2}')
...
if __name__ == '__main__':
    ...
    all_possilbe_query = Brutal_Force(query)

    print("posssible query : ")
    with open('BF_possible_query.csv', 'w', newline='') as csvfile:
        # 建立 CSV 檔寫入器
        writer = csv.writer(csvfile)
        print("---------------------------")
        for quer in all_possilbe_query:
            writer.writerow([quer])
            print(quer)
            print("---------------------------")
